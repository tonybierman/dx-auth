...
//! scoped under `arium_dioxus::ui::components::*` so consumers can reach them by
//! name, although most consumers only need `arium_dioxus::ui::LoginPanel`.
//!
//! Each primitive is a thin, scoped copy of the matching widget from the
//! `dioxus-primitives` catalog with the auth CSS module classes wired in.
